...
def generate_random_base64(nb_bytes: int = 110880) -> str:
    # 110880 == 77 * 768 * 15 / 8
    length = nb_bytes * 8 // 6
...
def unpack_binary_key_into_binary_float_array(key: bytes, data_size=(77*768)*2) -> bytes:
    data = bytearray(data_size * [0])
    datum = 0
    datum_bit_i = 0
    data_i = 0
    for key_byte_bit_i in range(0, 8*len(key), 15):
        key_byte_i = key_byte_bit_i // 8
        datum = 0
        for datum_bit_i in range(15):
            key_bit_i = key_byte_bit_i + datum_bit_i
            if(key_bit_i < len(key)*8):
                datum = (datum << 1) | ((key[key_byte_i] >> (7 - key_bit_i % 8)) & 1)
